...
| Template hero | centered headline+sub+buttons, 100vh | break the spine: offset grid, strong edge |
| Default type stack | Inter, flat 14-18px, 700 headings | editorial contrast; ≥1.25x; 60-76ch |
| Anti-reference echo | design guessable from a banned list | restart with a direction contract: register+mode+scene |

## Invocation

**Model-invoked:** UI in view → flag tells in one line, offer the command; HTML reachable → run `detect`. **User-invoked:** `shape`/`craft` grill first - never assume (reference/shape.md); `audit`/`deslop` take a target.
...
- **Icons:** one real set (Lucide, Phosphor, Tabler, Iconoir, Humbleicons, Flowbite). Fetch it; never hand-draw mascots, never emoji.
...
- **Font:** pick as a physical object (museum caption, terminal manual); reject reflex defaults (Inter, Fraunces, Space Grotesk - register.md).
- **Reference hunt:** web-search 2-3 inspirations that fit the brief (layout, type, palette); cite them.

## Modes & register

Register: brand or product (register.md). Mode: Persuade/Operate/Read/Experience, from the surface (modes.md).
