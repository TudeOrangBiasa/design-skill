...
| registry lock | The rule count is test-locked; bumping it requires a test per new rule | `scripts/detector.test.mjs` ("rule registry size is locked", 52) |
| command | One of the five user commands with ability flags | `scripts/command-metadata.json` (validated by `scripts/validate-catalog.mjs`) |
...
| checklist core floor | Mandatory pre-ship UI checks | `reference/checklist.md` (703-check deep pass: `datasets/checklist-catalog.md`) |
...
| eval family | A measured skill-lift scenario: prompt + fixtures + judge assertions | `evals/evals.json` (66 evals; guard: `scripts/validate-evals.mjs`) |
| dataset | Unshipped research corpus or big reference data | `datasets/` (checklist-catalog, research notes) |
...
- Training data = evals + fixtures (`scripts/fixtures/*`), gated by
  `scripts/validate-evals.mjs` before any API spend.
