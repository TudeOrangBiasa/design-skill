...
3. **Mechanical sweep:** run `node {{scripts_path}}/detector.mjs <target>` on the changed files first (deterministic tells, no LLM), fix what is mechanical, then run `/design audit --smell` and `/design audit --checkup` and pass remaining findings into the review.
